...
#
# Run Battle For Wesnoth (Docker).sh
#
...
set -e
apt-get update
apt-get install -y --no-install-recommends wesnoth
...
ENTRYPOINT ["/usr/games/wesnoth"]
FINAL_EOF
)

# Build the Docker image directly from the here document
echo "$DOCKERFILE_CONTENT" | docker build -t local/wesnoth - 2>&1
