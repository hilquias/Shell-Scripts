...
set -e
apt-get update
apt-get install -y --no-install-recommends freeciv
...
ENTRYPOINT ["/usr/games/freeciv-gtk3.22"]
FINAL_EOF
)

# Build the Docker image directly from the here document
echo "$DOCKERFILE_CONTENT" | docker build -t local/freeciv - 2>&1
